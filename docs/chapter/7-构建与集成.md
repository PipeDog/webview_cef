...
所有平台的下载脚本均支持通过 `~/.cef/tar/` 系统级缓存目录实现离线加速：将对应平台的 CEF tarball 预先放入 `~/.cef/tar/` 后，脚本会自动跳过 CDN 下载，直接使用本地文件。缓存路径与项目位置无关，任何集成方式（本地路径、Git 依赖、pub 依赖）均生效。可通过环境变量 `CEF_TAR_CACHE_DIR` 自定义缓存路径（可选）。详见 `cef_tar/README.md`。
